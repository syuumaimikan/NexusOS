<#
.SYNOPSIS
    Asks the machine which Linux system calls it does not have, and prints the
    list.
...
.DESCRIPTION
    ...
    "implement Linux" into a finite list, but a list written from memory is a
    list of what somebody happened to remember.

    So this asks the machine. `tools/nexus-probe` is a Linux program that makes
    each call with arguments chosen to be harmless -- a file descriptor of -1,
    or a null pointer -- and looks only at whether the answer is ENOSYS.

    `getpid` is on its list as a control. If that comes back missing, the probe
    is wrong rather than the kernel, and this script fails on it.
...
$Log = Join-Path $BuildDir 'probe-test.log'
...
$FirmwareVars = Join-Path $BuildDir 'vars-probe.fd'
...
$LockFile = Join-Path $LockDir 'CLAUDE-MACHINE-004.json'
...
$QemuErrors = Join-Path $BuildDir 'probe-qemu.err'
...
try {
    ...
    try {
        ...
        Write-Host '==> Typing "prob"' -ForegroundColor Cyan
        Send-Keys @('p', 'r', 'o', 'b')
        ...
        Write-Host '==> Waiting for the probe to finish' -ForegroundColor Cyan
        if (-not (Wait-For -Text ' present, ' -Seconds $Timeout)) {
            $failures += 'the probe never said what it found'
        }
        ...
        if ($Shot) {
            ...
            $Ppm = Join-Path $BuildDir 'probe.ppm'
            try {
...
foreach ($expected in @(
        'launch: asked for prob',
        'compositor: started the Linux ABI probe, and lent it nothing at all',
        'probe: asking this machine which calls it does not have',
        'probe: have getpid'
    )) {
...
foreach ($bad in @('probe: FAILED', 'probe: PANIC', 'compositor: FAILED', 'KERNEL PANIC')) {
...
if ($failures.Count -eq 0) {
    Write-Host 'The machine said which calls it does not have.' -ForegroundColor Green
    exit 0
} else {
