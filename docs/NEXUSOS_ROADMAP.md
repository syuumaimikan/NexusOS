...
| ~~Japanese input~~ | done | Romaji to kana is a table; kana to kanji is a dictionary, and there is one -- 225 readings written out by hand in `shared/nexus-ime/src/dictionary.rs`, against the hundred thousand a real input method ships. The size is stated rather than implied, and the consequence is stated where somebody meets it: a word that is not in the table converts to itself, in kana or katakana, which is a real answer. Space converts and cycles, escape puts the kana back, return commits. The font had to learn 358 kanji at the same time -- conversion produced 漢字 and drew it as a box and a character, because the face carried the second and not the first. See [japanese-input.md](japanese-input.md). |
...
| ~~Standard output for programs~~ | done | `ls` is a program now -- `user/nexus-ls` -- and the terminal reads what it writes off a channel while it runs. The estimate above said three inventions were needed: a directory a child inherits, a standard output and a pipe. It was one line: the spawn service already gave every program a channel and a message could already carry handles, and the service was throwing the caller's handles away. Whatever the asker attaches is now the child's, numbered from two. A pipe is two of those conventions meeting and nothing else. See [standard-output.md](standard-output.md). |
| ~~A real audio card~~ | done | AC'97: a DMA engine, a descriptor list and a codec, exactly as the estimate said. Sixteen-bit stereo at 48 kHz through `kernel/nexus-kernel/src/drivers/ac97.rs`; the speaker is still there and is what a machine without a card falls back to. Checked without listening to anything: the controller publishes which descriptor it is reading and how much of it is left, those move only because the engine is reading memory, and `tone` returns false when they do not -- so "played the chime **through the sound card**" is a sentence that has the other ending when the driver is wrong. The test machine is given `-audiodev none`, so the card is entirely real to the guest and the samples go nowhere on the host. See [sound-card.md](sound-card.md). |
| ~~A language that runs on the machine~~ | done | **Nex** -- `shared/nexus-lang` and `user/nexus-nex`. Not Python and not C, and named so nobody arrives expecting either: CPython is six hundred thousand lines of C needing a C library under it, GCC is millions needing an assembler, a linker and a target description, and a thing called "Python" that ran a tenth of Python would be worse than nothing. Integers, strings, booleans, variables, `if`/`else`, `while`, functions with recursion, six built-ins, and errors that carry their line. A tree-walking interpreter with 35 tests on the host, plus writing a program in the shell and running it there. See [nex.md](nex.md). |
...
| ~~An interface programs write against~~ | done | `user/nexus-api`: starting a program, walking a path of several components, asking what the machine is doing. Asked for as "a Nexus API like the Windows API", and deliberately not that shape -- `CreateProcess` is something any process may call because it is a process, and here `Spawn::start` takes a spawn-service handle that a program was either lent or was not. A convenience over the handing, never a way around it. The terminal uses it, which is the only thing that makes it an API rather than a claim. See [nexus-api.md](nexus-api.md). |
